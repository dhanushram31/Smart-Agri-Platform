...
// Route to register seeds - requires auth
router.post('/register', auth, upload.single('image'), async (req, res) => {
    // derive creator from authenticated user, do NOT trust client
    const createdBy = req.user && (req.user.id || req.user._id);
    const createdByEmail = req.user && req.user.email;

    const { seedName, seedType, description } = req.body;
    ...
    try {
        ...
        if (!seedName || !seedType || !createdBy || !createdByEmail) {
            return res.status(400).json({ 
                ...
                details: 'seedName, seedType, and authenticated user info are required' 
            });
        }
