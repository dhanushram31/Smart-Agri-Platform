...
const Navbar = () => {
  ...
  const navigationItems = [
    ...
    { label: "Labour Hub", href: "/labour-hub" },
    { label: "Scheme Portal", href: "/scheme-portal" },
    {
      label: "Tools",
      href: "/tools",
      dropdown: [
                  ...
                  <div className={`dropdown-menu ${activeDropdown === index ? "show" : ""}`}>
                    <div className="dropdown-content">
                      {item.dropdown.map((dropdownItem, dropdownIndex) => (
                        <button key={dropdownIndex} onClick={() => navigateToPage(dropdownItem.href)} className="dropdown-link">
                          {dropdownItem.label}
                        </button>
                      ))}
                    </div>
        ...
        <div className="navbar-actions">
              ...
              <div className="calendar-header">
                <div className="calendar-nav-controls">
                  ...
                  <div className="calendar-month-year">
                    <h4>{new Date(currentCalendarYear, currentCalendarMonth).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</h4>
                  </div>
                ...
                <div className="calendar-today-section">
                  <span className="current-date">Today: {new Date().toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</span>
                  <button onClick={navigateToCurrentMonth} className="today-btn" title="Go to current month">Today</button>
                </div>
              </div>
                      ...
                      for (let i = 0; i < 42; i++) {
                        ...
                        days.push(
                          ...
                          >
                            ...
                            {noteCount > 0 && <span className="note-indicator">{noteCount}</span>}
                          </button>
                        )
                      }
                      return days
                      ...
                      {dateNotes[selectedDate] && dateNotes[selectedDate].length > 0 ? (
                        dateNotes[selectedDate].map(note => (
                          <div key={note.id} className={`note-item ${note.completed ? 'completed' : ''}`}>
                            <button onClick={() => toggleDateNoteComplete(selectedDate, note.id)} className="note-checkbox">
                              {note.completed ? '✅' : '◻️'}
                            </button>
                            <span className="note-text">{note.text}</span>
                            <button onClick={() => deleteDateNote(selectedDate, note.id)} className="delete-note-btn">🗑️</button>
                          </div>
                        ))
                      ) : (
                        !showDateNoteInput && <p className="no-notes">No activities for this date</p>
                      )}
                    </div>
                  </div>
                )}
          ...
          {user ? (
            ...
            <div className="auth-buttons">
              ...
              <button onClick={handleRegister} className="cta-btn">Get Started</button>
            </div>
          )}
        ...
        <div className="mobile-nav-content">
              ...
              <li key={index} className="mobile-nav-item">
                ...
                {item.dropdown && (
                  <div className={`mobile-dropdown ${activeDropdown === index ? "show" : ""}`}>
                    {item.dropdown.map((dropdownItem, dropdownIndex) => (
                      <button key={dropdownIndex} onClick={() => navigateToPage(dropdownItem.href)} className="mobile-dropdown-link">
                        {dropdownItem.label}
                      </button>
                    ))}
                  </div>
          ...
          <div className="mobile-actions">
            {user ? (
              ...
              <>
                <button onClick={handleLogin} className="mobile-login-btn">Login</button>
                <button onClick={handleRegister} className="mobile-cta-btn">Get Started</button>
              </>
            )}
          </div>
        </div>
